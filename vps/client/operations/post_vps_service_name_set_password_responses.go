// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/vps/models"
)

// PostVpsServiceNameSetPasswordReader is a Reader for the PostVpsServiceNameSetPassword structure.
type PostVpsServiceNameSetPasswordReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *PostVpsServiceNameSetPasswordReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewPostVpsServiceNameSetPasswordOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewPostVpsServiceNameSetPasswordDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewPostVpsServiceNameSetPasswordOK creates a PostVpsServiceNameSetPasswordOK with default headers values
func NewPostVpsServiceNameSetPasswordOK() *PostVpsServiceNameSetPasswordOK {
	return &PostVpsServiceNameSetPasswordOK{}
}

/*PostVpsServiceNameSetPasswordOK handles this case with default header values.

description of 'vps.Task' response
*/
type PostVpsServiceNameSetPasswordOK struct {
	Payload *models.VpsTask
}

func (o *PostVpsServiceNameSetPasswordOK) Error() string {
	return fmt.Sprintf("[POST /vps/{serviceName}/setPassword][%d] postVpsServiceNameSetPasswordOK  %+v", 200, o.Payload)
}

func (o *PostVpsServiceNameSetPasswordOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.VpsTask)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewPostVpsServiceNameSetPasswordDefault creates a PostVpsServiceNameSetPasswordDefault with default headers values
func NewPostVpsServiceNameSetPasswordDefault(code int) *PostVpsServiceNameSetPasswordDefault {
	return &PostVpsServiceNameSetPasswordDefault{
		_statusCode: code,
	}
}

/*PostVpsServiceNameSetPasswordDefault handles this case with default header values.

Unexpected error
*/
type PostVpsServiceNameSetPasswordDefault struct {
	_statusCode int

	Payload *models.PostVpsServiceNameSetPasswordDefaultBody
}

// Code gets the status code for the post vps service name set password default response
func (o *PostVpsServiceNameSetPasswordDefault) Code() int {
	return o._statusCode
}

func (o *PostVpsServiceNameSetPasswordDefault) Error() string {
	return fmt.Sprintf("[POST /vps/{serviceName}/setPassword][%d] PostVpsServiceNameSetPassword default  %+v", o._statusCode, o.Payload)
}

func (o *PostVpsServiceNameSetPasswordDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.PostVpsServiceNameSetPasswordDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
