// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/vps/models"
)

// GetVpsServiceNameModelsReader is a Reader for the GetVpsServiceNameModels structure.
type GetVpsServiceNameModelsReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *GetVpsServiceNameModelsReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewGetVpsServiceNameModelsOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewGetVpsServiceNameModelsDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewGetVpsServiceNameModelsOK creates a GetVpsServiceNameModelsOK with default headers values
func NewGetVpsServiceNameModelsOK() *GetVpsServiceNameModelsOK {
	return &GetVpsServiceNameModelsOK{}
}

/*GetVpsServiceNameModelsOK handles this case with default header values.

return value
*/
type GetVpsServiceNameModelsOK struct {
	Payload models.GetVpsServiceNameModelsOKBody
}

func (o *GetVpsServiceNameModelsOK) Error() string {
	return fmt.Sprintf("[GET /vps/{serviceName}/models][%d] getVpsServiceNameModelsOK  %+v", 200, o.Payload)
}

func (o *GetVpsServiceNameModelsOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	// response payload
	if err := consumer.Consume(response.Body(), &o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewGetVpsServiceNameModelsDefault creates a GetVpsServiceNameModelsDefault with default headers values
func NewGetVpsServiceNameModelsDefault(code int) *GetVpsServiceNameModelsDefault {
	return &GetVpsServiceNameModelsDefault{
		_statusCode: code,
	}
}

/*GetVpsServiceNameModelsDefault handles this case with default header values.

Unexpected error
*/
type GetVpsServiceNameModelsDefault struct {
	_statusCode int

	Payload *models.GetVpsServiceNameModelsDefaultBody
}

// Code gets the status code for the get vps service name models default response
func (o *GetVpsServiceNameModelsDefault) Code() int {
	return o._statusCode
}

func (o *GetVpsServiceNameModelsDefault) Error() string {
	return fmt.Sprintf("[GET /vps/{serviceName}/models][%d] GetVpsServiceNameModels default  %+v", o._statusCode, o.Payload)
}

func (o *GetVpsServiceNameModelsDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.GetVpsServiceNameModelsDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
