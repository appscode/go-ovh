// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/vps/models"
)

// GetVpsServiceNameTemplatesIDReader is a Reader for the GetVpsServiceNameTemplatesID structure.
type GetVpsServiceNameTemplatesIDReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *GetVpsServiceNameTemplatesIDReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewGetVpsServiceNameTemplatesIDOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewGetVpsServiceNameTemplatesIDDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewGetVpsServiceNameTemplatesIDOK creates a GetVpsServiceNameTemplatesIDOK with default headers values
func NewGetVpsServiceNameTemplatesIDOK() *GetVpsServiceNameTemplatesIDOK {
	return &GetVpsServiceNameTemplatesIDOK{}
}

/*GetVpsServiceNameTemplatesIDOK handles this case with default header values.

description of 'vps.Template' response
*/
type GetVpsServiceNameTemplatesIDOK struct {
	Payload *models.VpsTemplate
}

func (o *GetVpsServiceNameTemplatesIDOK) Error() string {
	return fmt.Sprintf("[GET /vps/{serviceName}/templates/{id}][%d] getVpsServiceNameTemplatesIdOK  %+v", 200, o.Payload)
}

func (o *GetVpsServiceNameTemplatesIDOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.VpsTemplate)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewGetVpsServiceNameTemplatesIDDefault creates a GetVpsServiceNameTemplatesIDDefault with default headers values
func NewGetVpsServiceNameTemplatesIDDefault(code int) *GetVpsServiceNameTemplatesIDDefault {
	return &GetVpsServiceNameTemplatesIDDefault{
		_statusCode: code,
	}
}

/*GetVpsServiceNameTemplatesIDDefault handles this case with default header values.

Unexpected error
*/
type GetVpsServiceNameTemplatesIDDefault struct {
	_statusCode int

	Payload *models.GetVpsServiceNameTemplatesIDDefaultBody
}

// Code gets the status code for the get vps service name templates ID default response
func (o *GetVpsServiceNameTemplatesIDDefault) Code() int {
	return o._statusCode
}

func (o *GetVpsServiceNameTemplatesIDDefault) Error() string {
	return fmt.Sprintf("[GET /vps/{serviceName}/templates/{id}][%d] GetVpsServiceNameTemplatesID default  %+v", o._statusCode, o.Payload)
}

func (o *GetVpsServiceNameTemplatesIDDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.GetVpsServiceNameTemplatesIDDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
