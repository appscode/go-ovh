// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/vps/models"
)

// PostVpsServiceNameOpenConsoleAccessReader is a Reader for the PostVpsServiceNameOpenConsoleAccess structure.
type PostVpsServiceNameOpenConsoleAccessReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *PostVpsServiceNameOpenConsoleAccessReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewPostVpsServiceNameOpenConsoleAccessOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewPostVpsServiceNameOpenConsoleAccessDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewPostVpsServiceNameOpenConsoleAccessOK creates a PostVpsServiceNameOpenConsoleAccessOK with default headers values
func NewPostVpsServiceNameOpenConsoleAccessOK() *PostVpsServiceNameOpenConsoleAccessOK {
	return &PostVpsServiceNameOpenConsoleAccessOK{}
}

/*PostVpsServiceNameOpenConsoleAccessOK handles this case with default header values.

description of 'vps.Vnc' response
*/
type PostVpsServiceNameOpenConsoleAccessOK struct {
	Payload *models.VpsVnc
}

func (o *PostVpsServiceNameOpenConsoleAccessOK) Error() string {
	return fmt.Sprintf("[POST /vps/{serviceName}/openConsoleAccess][%d] postVpsServiceNameOpenConsoleAccessOK  %+v", 200, o.Payload)
}

func (o *PostVpsServiceNameOpenConsoleAccessOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.VpsVnc)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewPostVpsServiceNameOpenConsoleAccessDefault creates a PostVpsServiceNameOpenConsoleAccessDefault with default headers values
func NewPostVpsServiceNameOpenConsoleAccessDefault(code int) *PostVpsServiceNameOpenConsoleAccessDefault {
	return &PostVpsServiceNameOpenConsoleAccessDefault{
		_statusCode: code,
	}
}

/*PostVpsServiceNameOpenConsoleAccessDefault handles this case with default header values.

Unexpected error
*/
type PostVpsServiceNameOpenConsoleAccessDefault struct {
	_statusCode int

	Payload *models.PostVpsServiceNameOpenConsoleAccessDefaultBody
}

// Code gets the status code for the post vps service name open console access default response
func (o *PostVpsServiceNameOpenConsoleAccessDefault) Code() int {
	return o._statusCode
}

func (o *PostVpsServiceNameOpenConsoleAccessDefault) Error() string {
	return fmt.Sprintf("[POST /vps/{serviceName}/openConsoleAccess][%d] PostVpsServiceNameOpenConsoleAccess default  %+v", o._statusCode, o.Payload)
}

func (o *PostVpsServiceNameOpenConsoleAccessDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.PostVpsServiceNameOpenConsoleAccessDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
