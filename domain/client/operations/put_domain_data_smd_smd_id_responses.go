// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/domain/models"
)

// PutDomainDataSmdSmdIDReader is a Reader for the PutDomainDataSmdSmdID structure.
type PutDomainDataSmdSmdIDReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *PutDomainDataSmdSmdIDReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewPutDomainDataSmdSmdIDOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewPutDomainDataSmdSmdIDDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewPutDomainDataSmdSmdIDOK creates a PutDomainDataSmdSmdIDOK with default headers values
func NewPutDomainDataSmdSmdIDOK() *PutDomainDataSmdSmdIDOK {
	return &PutDomainDataSmdSmdIDOK{}
}

/*PutDomainDataSmdSmdIDOK handles this case with default header values.

description of 'domain.Data.Smd' response
*/
type PutDomainDataSmdSmdIDOK struct {
	Payload *models.DomainDataSmd
}

func (o *PutDomainDataSmdSmdIDOK) Error() string {
	return fmt.Sprintf("[PUT /domain/data/smd/{smdId}][%d] putDomainDataSmdSmdIdOK  %+v", 200, o.Payload)
}

func (o *PutDomainDataSmdSmdIDOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.DomainDataSmd)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewPutDomainDataSmdSmdIDDefault creates a PutDomainDataSmdSmdIDDefault with default headers values
func NewPutDomainDataSmdSmdIDDefault(code int) *PutDomainDataSmdSmdIDDefault {
	return &PutDomainDataSmdSmdIDDefault{
		_statusCode: code,
	}
}

/*PutDomainDataSmdSmdIDDefault handles this case with default header values.

Unexpected error
*/
type PutDomainDataSmdSmdIDDefault struct {
	_statusCode int

	Payload *models.PutDomainDataSmdSmdIDDefaultBody
}

// Code gets the status code for the put domain data smd smd ID default response
func (o *PutDomainDataSmdSmdIDDefault) Code() int {
	return o._statusCode
}

func (o *PutDomainDataSmdSmdIDDefault) Error() string {
	return fmt.Sprintf("[PUT /domain/data/smd/{smdId}][%d] PutDomainDataSmdSmdID default  %+v", o._statusCode, o.Payload)
}

func (o *PutDomainDataSmdSmdIDDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.PutDomainDataSmdSmdIDDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
