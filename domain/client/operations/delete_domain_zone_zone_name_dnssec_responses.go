// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/domain/models"
)

// DeleteDomainZoneZoneNameDnssecReader is a Reader for the DeleteDomainZoneZoneNameDnssec structure.
type DeleteDomainZoneZoneNameDnssecReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *DeleteDomainZoneZoneNameDnssecReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewDeleteDomainZoneZoneNameDnssecOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewDeleteDomainZoneZoneNameDnssecDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewDeleteDomainZoneZoneNameDnssecOK creates a DeleteDomainZoneZoneNameDnssecOK with default headers values
func NewDeleteDomainZoneZoneNameDnssecOK() *DeleteDomainZoneZoneNameDnssecOK {
	return &DeleteDomainZoneZoneNameDnssecOK{}
}

/*DeleteDomainZoneZoneNameDnssecOK handles this case with default header values.

return 'void'
*/
type DeleteDomainZoneZoneNameDnssecOK struct {
}

func (o *DeleteDomainZoneZoneNameDnssecOK) Error() string {
	return fmt.Sprintf("[DELETE /domain/zone/{zoneName}/dnssec][%d] deleteDomainZoneZoneNameDnssecOK ", 200)
}

func (o *DeleteDomainZoneZoneNameDnssecOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	return nil
}

// NewDeleteDomainZoneZoneNameDnssecDefault creates a DeleteDomainZoneZoneNameDnssecDefault with default headers values
func NewDeleteDomainZoneZoneNameDnssecDefault(code int) *DeleteDomainZoneZoneNameDnssecDefault {
	return &DeleteDomainZoneZoneNameDnssecDefault{
		_statusCode: code,
	}
}

/*DeleteDomainZoneZoneNameDnssecDefault handles this case with default header values.

Unexpected error
*/
type DeleteDomainZoneZoneNameDnssecDefault struct {
	_statusCode int

	Payload *models.DeleteDomainZoneZoneNameDnssecDefaultBody
}

// Code gets the status code for the delete domain zone zone name dnssec default response
func (o *DeleteDomainZoneZoneNameDnssecDefault) Code() int {
	return o._statusCode
}

func (o *DeleteDomainZoneZoneNameDnssecDefault) Error() string {
	return fmt.Sprintf("[DELETE /domain/zone/{zoneName}/dnssec][%d] DeleteDomainZoneZoneNameDnssec default  %+v", o._statusCode, o.Payload)
}

func (o *DeleteDomainZoneZoneNameDnssecDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.DeleteDomainZoneZoneNameDnssecDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
