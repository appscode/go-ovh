// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/me/models"
)

// GetMeTaskContactChangeIDReader is a Reader for the GetMeTaskContactChangeID structure.
type GetMeTaskContactChangeIDReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *GetMeTaskContactChangeIDReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewGetMeTaskContactChangeIDOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewGetMeTaskContactChangeIDDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewGetMeTaskContactChangeIDOK creates a GetMeTaskContactChangeIDOK with default headers values
func NewGetMeTaskContactChangeIDOK() *GetMeTaskContactChangeIDOK {
	return &GetMeTaskContactChangeIDOK{}
}

/*GetMeTaskContactChangeIDOK handles this case with default header values.

description of 'nichandle.ContactChange.Task' response
*/
type GetMeTaskContactChangeIDOK struct {
	Payload *models.NichandleContactChangeTask
}

func (o *GetMeTaskContactChangeIDOK) Error() string {
	return fmt.Sprintf("[GET /me/task/contactChange/{id}][%d] getMeTaskContactChangeIdOK  %+v", 200, o.Payload)
}

func (o *GetMeTaskContactChangeIDOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.NichandleContactChangeTask)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewGetMeTaskContactChangeIDDefault creates a GetMeTaskContactChangeIDDefault with default headers values
func NewGetMeTaskContactChangeIDDefault(code int) *GetMeTaskContactChangeIDDefault {
	return &GetMeTaskContactChangeIDDefault{
		_statusCode: code,
	}
}

/*GetMeTaskContactChangeIDDefault handles this case with default header values.

Unexpected error
*/
type GetMeTaskContactChangeIDDefault struct {
	_statusCode int

	Payload *models.GetMeTaskContactChangeIDDefaultBody
}

// Code gets the status code for the get me task contact change ID default response
func (o *GetMeTaskContactChangeIDDefault) Code() int {
	return o._statusCode
}

func (o *GetMeTaskContactChangeIDDefault) Error() string {
	return fmt.Sprintf("[GET /me/task/contactChange/{id}][%d] GetMeTaskContactChangeID default  %+v", o._statusCode, o.Payload)
}

func (o *GetMeTaskContactChangeIDDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.GetMeTaskContactChangeIDDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
