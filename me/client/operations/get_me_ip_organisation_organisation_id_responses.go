// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/me/models"
)

// GetMeIPOrganisationOrganisationIDReader is a Reader for the GetMeIPOrganisationOrganisationID structure.
type GetMeIPOrganisationOrganisationIDReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *GetMeIPOrganisationOrganisationIDReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewGetMeIPOrganisationOrganisationIDOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewGetMeIPOrganisationOrganisationIDDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewGetMeIPOrganisationOrganisationIDOK creates a GetMeIPOrganisationOrganisationIDOK with default headers values
func NewGetMeIPOrganisationOrganisationIDOK() *GetMeIPOrganisationOrganisationIDOK {
	return &GetMeIPOrganisationOrganisationIDOK{}
}

/*GetMeIPOrganisationOrganisationIDOK handles this case with default header values.

description of 'nichandle.Ipv4Org' response
*/
type GetMeIPOrganisationOrganisationIDOK struct {
	Payload *models.NichandleIPV4Org
}

func (o *GetMeIPOrganisationOrganisationIDOK) Error() string {
	return fmt.Sprintf("[GET /me/ipOrganisation/{organisationId}][%d] getMeIpOrganisationOrganisationIdOK  %+v", 200, o.Payload)
}

func (o *GetMeIPOrganisationOrganisationIDOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.NichandleIPV4Org)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewGetMeIPOrganisationOrganisationIDDefault creates a GetMeIPOrganisationOrganisationIDDefault with default headers values
func NewGetMeIPOrganisationOrganisationIDDefault(code int) *GetMeIPOrganisationOrganisationIDDefault {
	return &GetMeIPOrganisationOrganisationIDDefault{
		_statusCode: code,
	}
}

/*GetMeIPOrganisationOrganisationIDDefault handles this case with default header values.

Unexpected error
*/
type GetMeIPOrganisationOrganisationIDDefault struct {
	_statusCode int

	Payload *models.GetMeIPOrganisationOrganisationIDDefaultBody
}

// Code gets the status code for the get me IP organisation organisation ID default response
func (o *GetMeIPOrganisationOrganisationIDDefault) Code() int {
	return o._statusCode
}

func (o *GetMeIPOrganisationOrganisationIDDefault) Error() string {
	return fmt.Sprintf("[GET /me/ipOrganisation/{organisationId}][%d] GetMeIPOrganisationOrganisationID default  %+v", o._statusCode, o.Payload)
}

func (o *GetMeIPOrganisationOrganisationIDDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.GetMeIPOrganisationOrganisationIDDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
