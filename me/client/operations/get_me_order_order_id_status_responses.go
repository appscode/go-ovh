// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/me/models"
)

// GetMeOrderOrderIDStatusReader is a Reader for the GetMeOrderOrderIDStatus structure.
type GetMeOrderOrderIDStatusReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *GetMeOrderOrderIDStatusReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewGetMeOrderOrderIDStatusOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewGetMeOrderOrderIDStatusDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewGetMeOrderOrderIDStatusOK creates a GetMeOrderOrderIDStatusOK with default headers values
func NewGetMeOrderOrderIDStatusOK() *GetMeOrderOrderIDStatusOK {
	return &GetMeOrderOrderIDStatusOK{}
}

/*GetMeOrderOrderIDStatusOK handles this case with default header values.

return value
*/
type GetMeOrderOrderIDStatusOK struct {
	Payload string
}

func (o *GetMeOrderOrderIDStatusOK) Error() string {
	return fmt.Sprintf("[GET /me/order/{orderId}/status][%d] getMeOrderOrderIdStatusOK  %+v", 200, o.Payload)
}

func (o *GetMeOrderOrderIDStatusOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	// response payload
	if err := consumer.Consume(response.Body(), &o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewGetMeOrderOrderIDStatusDefault creates a GetMeOrderOrderIDStatusDefault with default headers values
func NewGetMeOrderOrderIDStatusDefault(code int) *GetMeOrderOrderIDStatusDefault {
	return &GetMeOrderOrderIDStatusDefault{
		_statusCode: code,
	}
}

/*GetMeOrderOrderIDStatusDefault handles this case with default header values.

Unexpected error
*/
type GetMeOrderOrderIDStatusDefault struct {
	_statusCode int

	Payload *models.GetMeOrderOrderIDStatusDefaultBody
}

// Code gets the status code for the get me order order ID status default response
func (o *GetMeOrderOrderIDStatusDefault) Code() int {
	return o._statusCode
}

func (o *GetMeOrderOrderIDStatusDefault) Error() string {
	return fmt.Sprintf("[GET /me/order/{orderId}/status][%d] GetMeOrderOrderIDStatus default  %+v", o._statusCode, o.Payload)
}

func (o *GetMeOrderOrderIDStatusDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.GetMeOrderOrderIDStatusDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
