// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/me/models"
)

// GetMeAccessRestrictionDeveloperModeReader is a Reader for the GetMeAccessRestrictionDeveloperMode structure.
type GetMeAccessRestrictionDeveloperModeReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *GetMeAccessRestrictionDeveloperModeReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewGetMeAccessRestrictionDeveloperModeOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewGetMeAccessRestrictionDeveloperModeDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewGetMeAccessRestrictionDeveloperModeOK creates a GetMeAccessRestrictionDeveloperModeOK with default headers values
func NewGetMeAccessRestrictionDeveloperModeOK() *GetMeAccessRestrictionDeveloperModeOK {
	return &GetMeAccessRestrictionDeveloperModeOK{}
}

/*GetMeAccessRestrictionDeveloperModeOK handles this case with default header values.

description of 'nichandle.DeveloperModeRestriction' response
*/
type GetMeAccessRestrictionDeveloperModeOK struct {
	Payload *models.NichandleDeveloperModeRestriction
}

func (o *GetMeAccessRestrictionDeveloperModeOK) Error() string {
	return fmt.Sprintf("[GET /me/accessRestriction/developerMode][%d] getMeAccessRestrictionDeveloperModeOK  %+v", 200, o.Payload)
}

func (o *GetMeAccessRestrictionDeveloperModeOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.NichandleDeveloperModeRestriction)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewGetMeAccessRestrictionDeveloperModeDefault creates a GetMeAccessRestrictionDeveloperModeDefault with default headers values
func NewGetMeAccessRestrictionDeveloperModeDefault(code int) *GetMeAccessRestrictionDeveloperModeDefault {
	return &GetMeAccessRestrictionDeveloperModeDefault{
		_statusCode: code,
	}
}

/*GetMeAccessRestrictionDeveloperModeDefault handles this case with default header values.

Unexpected error
*/
type GetMeAccessRestrictionDeveloperModeDefault struct {
	_statusCode int

	Payload *models.GetMeAccessRestrictionDeveloperModeDefaultBody
}

// Code gets the status code for the get me access restriction developer mode default response
func (o *GetMeAccessRestrictionDeveloperModeDefault) Code() int {
	return o._statusCode
}

func (o *GetMeAccessRestrictionDeveloperModeDefault) Error() string {
	return fmt.Sprintf("[GET /me/accessRestriction/developerMode][%d] GetMeAccessRestrictionDeveloperMode default  %+v", o._statusCode, o.Payload)
}

func (o *GetMeAccessRestrictionDeveloperModeDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.GetMeAccessRestrictionDeveloperModeDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
