// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/me/models"
)

// GetMeContactContactIDReader is a Reader for the GetMeContactContactID structure.
type GetMeContactContactIDReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *GetMeContactContactIDReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewGetMeContactContactIDOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewGetMeContactContactIDDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewGetMeContactContactIDOK creates a GetMeContactContactIDOK with default headers values
func NewGetMeContactContactIDOK() *GetMeContactContactIDOK {
	return &GetMeContactContactIDOK{}
}

/*GetMeContactContactIDOK handles this case with default header values.

description of 'contact.Contact' response
*/
type GetMeContactContactIDOK struct {
	Payload *models.ContactContact
}

func (o *GetMeContactContactIDOK) Error() string {
	return fmt.Sprintf("[GET /me/contact/{contactId}][%d] getMeContactContactIdOK  %+v", 200, o.Payload)
}

func (o *GetMeContactContactIDOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.ContactContact)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewGetMeContactContactIDDefault creates a GetMeContactContactIDDefault with default headers values
func NewGetMeContactContactIDDefault(code int) *GetMeContactContactIDDefault {
	return &GetMeContactContactIDDefault{
		_statusCode: code,
	}
}

/*GetMeContactContactIDDefault handles this case with default header values.

Unexpected error
*/
type GetMeContactContactIDDefault struct {
	_statusCode int

	Payload *models.GetMeContactContactIDDefaultBody
}

// Code gets the status code for the get me contact contact ID default response
func (o *GetMeContactContactIDDefault) Code() int {
	return o._statusCode
}

func (o *GetMeContactContactIDDefault) Error() string {
	return fmt.Sprintf("[GET /me/contact/{contactId}][%d] GetMeContactContactID default  %+v", o._statusCode, o.Payload)
}

func (o *GetMeContactContactIDDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.GetMeContactContactIDDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
