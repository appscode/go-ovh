// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/me/models"
)

// PostMePasswordRecoverReader is a Reader for the PostMePasswordRecover structure.
type PostMePasswordRecoverReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *PostMePasswordRecoverReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewPostMePasswordRecoverOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewPostMePasswordRecoverDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewPostMePasswordRecoverOK creates a PostMePasswordRecoverOK with default headers values
func NewPostMePasswordRecoverOK() *PostMePasswordRecoverOK {
	return &PostMePasswordRecoverOK{}
}

/*PostMePasswordRecoverOK handles this case with default header values.

return 'void'
*/
type PostMePasswordRecoverOK struct {
}

func (o *PostMePasswordRecoverOK) Error() string {
	return fmt.Sprintf("[POST /me/passwordRecover][%d] postMePasswordRecoverOK ", 200)
}

func (o *PostMePasswordRecoverOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	return nil
}

// NewPostMePasswordRecoverDefault creates a PostMePasswordRecoverDefault with default headers values
func NewPostMePasswordRecoverDefault(code int) *PostMePasswordRecoverDefault {
	return &PostMePasswordRecoverDefault{
		_statusCode: code,
	}
}

/*PostMePasswordRecoverDefault handles this case with default header values.

Unexpected error
*/
type PostMePasswordRecoverDefault struct {
	_statusCode int

	Payload *models.PostMePasswordRecoverDefaultBody
}

// Code gets the status code for the post me password recover default response
func (o *PostMePasswordRecoverDefault) Code() int {
	return o._statusCode
}

func (o *PostMePasswordRecoverDefault) Error() string {
	return fmt.Sprintf("[POST /me/passwordRecover][%d] PostMePasswordRecover default  %+v", o._statusCode, o.Payload)
}

func (o *PostMePasswordRecoverDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.PostMePasswordRecoverDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
