// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/me/models"
)

// GetMeOrderOrderIDPaymentMeansReader is a Reader for the GetMeOrderOrderIDPaymentMeans structure.
type GetMeOrderOrderIDPaymentMeansReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *GetMeOrderOrderIDPaymentMeansReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewGetMeOrderOrderIDPaymentMeansOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewGetMeOrderOrderIDPaymentMeansDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewGetMeOrderOrderIDPaymentMeansOK creates a GetMeOrderOrderIDPaymentMeansOK with default headers values
func NewGetMeOrderOrderIDPaymentMeansOK() *GetMeOrderOrderIDPaymentMeansOK {
	return &GetMeOrderOrderIDPaymentMeansOK{}
}

/*GetMeOrderOrderIDPaymentMeansOK handles this case with default header values.

description of 'billing.Order.PaymentMeans' response
*/
type GetMeOrderOrderIDPaymentMeansOK struct {
	Payload *models.BillingOrderPaymentMeans
}

func (o *GetMeOrderOrderIDPaymentMeansOK) Error() string {
	return fmt.Sprintf("[GET /me/order/{orderId}/paymentMeans][%d] getMeOrderOrderIdPaymentMeansOK  %+v", 200, o.Payload)
}

func (o *GetMeOrderOrderIDPaymentMeansOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.BillingOrderPaymentMeans)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewGetMeOrderOrderIDPaymentMeansDefault creates a GetMeOrderOrderIDPaymentMeansDefault with default headers values
func NewGetMeOrderOrderIDPaymentMeansDefault(code int) *GetMeOrderOrderIDPaymentMeansDefault {
	return &GetMeOrderOrderIDPaymentMeansDefault{
		_statusCode: code,
	}
}

/*GetMeOrderOrderIDPaymentMeansDefault handles this case with default header values.

Unexpected error
*/
type GetMeOrderOrderIDPaymentMeansDefault struct {
	_statusCode int

	Payload *models.GetMeOrderOrderIDPaymentMeansDefaultBody
}

// Code gets the status code for the get me order order ID payment means default response
func (o *GetMeOrderOrderIDPaymentMeansDefault) Code() int {
	return o._statusCode
}

func (o *GetMeOrderOrderIDPaymentMeansDefault) Error() string {
	return fmt.Sprintf("[GET /me/order/{orderId}/paymentMeans][%d] GetMeOrderOrderIDPaymentMeans default  %+v", o._statusCode, o.Payload)
}

func (o *GetMeOrderOrderIDPaymentMeansDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.GetMeOrderOrderIDPaymentMeansDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
