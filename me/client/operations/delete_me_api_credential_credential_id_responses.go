// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/me/models"
)

// DeleteMeAPICredentialCredentialIDReader is a Reader for the DeleteMeAPICredentialCredentialID structure.
type DeleteMeAPICredentialCredentialIDReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *DeleteMeAPICredentialCredentialIDReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewDeleteMeAPICredentialCredentialIDOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewDeleteMeAPICredentialCredentialIDDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewDeleteMeAPICredentialCredentialIDOK creates a DeleteMeAPICredentialCredentialIDOK with default headers values
func NewDeleteMeAPICredentialCredentialIDOK() *DeleteMeAPICredentialCredentialIDOK {
	return &DeleteMeAPICredentialCredentialIDOK{}
}

/*DeleteMeAPICredentialCredentialIDOK handles this case with default header values.

return 'void'
*/
type DeleteMeAPICredentialCredentialIDOK struct {
}

func (o *DeleteMeAPICredentialCredentialIDOK) Error() string {
	return fmt.Sprintf("[DELETE /me/api/credential/{credentialId}][%d] deleteMeApiCredentialCredentialIdOK ", 200)
}

func (o *DeleteMeAPICredentialCredentialIDOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	return nil
}

// NewDeleteMeAPICredentialCredentialIDDefault creates a DeleteMeAPICredentialCredentialIDDefault with default headers values
func NewDeleteMeAPICredentialCredentialIDDefault(code int) *DeleteMeAPICredentialCredentialIDDefault {
	return &DeleteMeAPICredentialCredentialIDDefault{
		_statusCode: code,
	}
}

/*DeleteMeAPICredentialCredentialIDDefault handles this case with default header values.

Unexpected error
*/
type DeleteMeAPICredentialCredentialIDDefault struct {
	_statusCode int

	Payload *models.DeleteMeAPICredentialCredentialIDDefaultBody
}

// Code gets the status code for the delete me API credential credential ID default response
func (o *DeleteMeAPICredentialCredentialIDDefault) Code() int {
	return o._statusCode
}

func (o *DeleteMeAPICredentialCredentialIDDefault) Error() string {
	return fmt.Sprintf("[DELETE /me/api/credential/{credentialId}][%d] DeleteMeAPICredentialCredentialID default  %+v", o._statusCode, o.Payload)
}

func (o *DeleteMeAPICredentialCredentialIDDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.DeleteMeAPICredentialCredentialIDDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
