// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/me/models"
)

// PostMeTelephonySettingsReader is a Reader for the PostMeTelephonySettings structure.
type PostMeTelephonySettingsReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *PostMeTelephonySettingsReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewPostMeTelephonySettingsOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewPostMeTelephonySettingsDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewPostMeTelephonySettingsOK creates a PostMeTelephonySettingsOK with default headers values
func NewPostMeTelephonySettingsOK() *PostMeTelephonySettingsOK {
	return &PostMeTelephonySettingsOK{}
}

/*PostMeTelephonySettingsOK handles this case with default header values.

return 'void'
*/
type PostMeTelephonySettingsOK struct {
}

func (o *PostMeTelephonySettingsOK) Error() string {
	return fmt.Sprintf("[POST /me/telephony/settings][%d] postMeTelephonySettingsOK ", 200)
}

func (o *PostMeTelephonySettingsOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	return nil
}

// NewPostMeTelephonySettingsDefault creates a PostMeTelephonySettingsDefault with default headers values
func NewPostMeTelephonySettingsDefault(code int) *PostMeTelephonySettingsDefault {
	return &PostMeTelephonySettingsDefault{
		_statusCode: code,
	}
}

/*PostMeTelephonySettingsDefault handles this case with default header values.

Unexpected error
*/
type PostMeTelephonySettingsDefault struct {
	_statusCode int

	Payload *models.PostMeTelephonySettingsDefaultBody
}

// Code gets the status code for the post me telephony settings default response
func (o *PostMeTelephonySettingsDefault) Code() int {
	return o._statusCode
}

func (o *PostMeTelephonySettingsDefault) Error() string {
	return fmt.Sprintf("[POST /me/telephony/settings][%d] PostMeTelephonySettings default  %+v", o._statusCode, o.Payload)
}

func (o *PostMeTelephonySettingsDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.PostMeTelephonySettingsDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
