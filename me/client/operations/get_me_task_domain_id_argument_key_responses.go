// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/me/models"
)

// GetMeTaskDomainIDArgumentKeyReader is a Reader for the GetMeTaskDomainIDArgumentKey structure.
type GetMeTaskDomainIDArgumentKeyReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *GetMeTaskDomainIDArgumentKeyReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewGetMeTaskDomainIDArgumentKeyOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewGetMeTaskDomainIDArgumentKeyDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewGetMeTaskDomainIDArgumentKeyOK creates a GetMeTaskDomainIDArgumentKeyOK with default headers values
func NewGetMeTaskDomainIDArgumentKeyOK() *GetMeTaskDomainIDArgumentKeyOK {
	return &GetMeTaskDomainIDArgumentKeyOK{}
}

/*GetMeTaskDomainIDArgumentKeyOK handles this case with default header values.

description of 'nichandle.DomainTaskArgument' response
*/
type GetMeTaskDomainIDArgumentKeyOK struct {
	Payload *models.NichandleDomainTaskArgument
}

func (o *GetMeTaskDomainIDArgumentKeyOK) Error() string {
	return fmt.Sprintf("[GET /me/task/domain/{id}/argument/{key}][%d] getMeTaskDomainIdArgumentKeyOK  %+v", 200, o.Payload)
}

func (o *GetMeTaskDomainIDArgumentKeyOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.NichandleDomainTaskArgument)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewGetMeTaskDomainIDArgumentKeyDefault creates a GetMeTaskDomainIDArgumentKeyDefault with default headers values
func NewGetMeTaskDomainIDArgumentKeyDefault(code int) *GetMeTaskDomainIDArgumentKeyDefault {
	return &GetMeTaskDomainIDArgumentKeyDefault{
		_statusCode: code,
	}
}

/*GetMeTaskDomainIDArgumentKeyDefault handles this case with default header values.

Unexpected error
*/
type GetMeTaskDomainIDArgumentKeyDefault struct {
	_statusCode int

	Payload *models.GetMeTaskDomainIDArgumentKeyDefaultBody
}

// Code gets the status code for the get me task domain ID argument key default response
func (o *GetMeTaskDomainIDArgumentKeyDefault) Code() int {
	return o._statusCode
}

func (o *GetMeTaskDomainIDArgumentKeyDefault) Error() string {
	return fmt.Sprintf("[GET /me/task/domain/{id}/argument/{key}][%d] GetMeTaskDomainIDArgumentKey default  %+v", o._statusCode, o.Payload)
}

func (o *GetMeTaskDomainIDArgumentKeyDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.GetMeTaskDomainIDArgumentKeyDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
