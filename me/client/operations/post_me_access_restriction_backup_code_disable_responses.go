// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/me/models"
)

// PostMeAccessRestrictionBackupCodeDisableReader is a Reader for the PostMeAccessRestrictionBackupCodeDisable structure.
type PostMeAccessRestrictionBackupCodeDisableReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *PostMeAccessRestrictionBackupCodeDisableReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewPostMeAccessRestrictionBackupCodeDisableOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewPostMeAccessRestrictionBackupCodeDisableDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewPostMeAccessRestrictionBackupCodeDisableOK creates a PostMeAccessRestrictionBackupCodeDisableOK with default headers values
func NewPostMeAccessRestrictionBackupCodeDisableOK() *PostMeAccessRestrictionBackupCodeDisableOK {
	return &PostMeAccessRestrictionBackupCodeDisableOK{}
}

/*PostMeAccessRestrictionBackupCodeDisableOK handles this case with default header values.

return 'void'
*/
type PostMeAccessRestrictionBackupCodeDisableOK struct {
}

func (o *PostMeAccessRestrictionBackupCodeDisableOK) Error() string {
	return fmt.Sprintf("[POST /me/accessRestriction/backupCode/disable][%d] postMeAccessRestrictionBackupCodeDisableOK ", 200)
}

func (o *PostMeAccessRestrictionBackupCodeDisableOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	return nil
}

// NewPostMeAccessRestrictionBackupCodeDisableDefault creates a PostMeAccessRestrictionBackupCodeDisableDefault with default headers values
func NewPostMeAccessRestrictionBackupCodeDisableDefault(code int) *PostMeAccessRestrictionBackupCodeDisableDefault {
	return &PostMeAccessRestrictionBackupCodeDisableDefault{
		_statusCode: code,
	}
}

/*PostMeAccessRestrictionBackupCodeDisableDefault handles this case with default header values.

Unexpected error
*/
type PostMeAccessRestrictionBackupCodeDisableDefault struct {
	_statusCode int

	Payload *models.PostMeAccessRestrictionBackupCodeDisableDefaultBody
}

// Code gets the status code for the post me access restriction backup code disable default response
func (o *PostMeAccessRestrictionBackupCodeDisableDefault) Code() int {
	return o._statusCode
}

func (o *PostMeAccessRestrictionBackupCodeDisableDefault) Error() string {
	return fmt.Sprintf("[POST /me/accessRestriction/backupCode/disable][%d] PostMeAccessRestrictionBackupCodeDisable default  %+v", o._statusCode, o.Payload)
}

func (o *PostMeAccessRestrictionBackupCodeDisableDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.PostMeAccessRestrictionBackupCodeDisableDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
