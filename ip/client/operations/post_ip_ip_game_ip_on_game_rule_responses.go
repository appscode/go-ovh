// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/ip/models"
)

// PostIPIPGameIPOnGameRuleReader is a Reader for the PostIPIPGameIPOnGameRule structure.
type PostIPIPGameIPOnGameRuleReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *PostIPIPGameIPOnGameRuleReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewPostIPIPGameIPOnGameRuleOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewPostIPIPGameIPOnGameRuleDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewPostIPIPGameIPOnGameRuleOK creates a PostIPIPGameIPOnGameRuleOK with default headers values
func NewPostIPIPGameIPOnGameRuleOK() *PostIPIPGameIPOnGameRuleOK {
	return &PostIPIPGameIPOnGameRuleOK{}
}

/*PostIPIPGameIPOnGameRuleOK handles this case with default header values.

description of 'ip.GameMitigationRule' response
*/
type PostIPIPGameIPOnGameRuleOK struct {
	Payload *models.IPGameMitigationRule
}

func (o *PostIPIPGameIPOnGameRuleOK) Error() string {
	return fmt.Sprintf("[POST /ip/{ip}/game/{ipOnGame}/rule][%d] postIpIpGameIpOnGameRuleOK  %+v", 200, o.Payload)
}

func (o *PostIPIPGameIPOnGameRuleOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.IPGameMitigationRule)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewPostIPIPGameIPOnGameRuleDefault creates a PostIPIPGameIPOnGameRuleDefault with default headers values
func NewPostIPIPGameIPOnGameRuleDefault(code int) *PostIPIPGameIPOnGameRuleDefault {
	return &PostIPIPGameIPOnGameRuleDefault{
		_statusCode: code,
	}
}

/*PostIPIPGameIPOnGameRuleDefault handles this case with default header values.

Unexpected error
*/
type PostIPIPGameIPOnGameRuleDefault struct {
	_statusCode int

	Payload *models.PostIPIPGameIPOnGameRuleDefaultBody
}

// Code gets the status code for the post IP IP game IP on game rule default response
func (o *PostIPIPGameIPOnGameRuleDefault) Code() int {
	return o._statusCode
}

func (o *PostIPIPGameIPOnGameRuleDefault) Error() string {
	return fmt.Sprintf("[POST /ip/{ip}/game/{ipOnGame}/rule][%d] PostIPIPGameIPOnGameRule default  %+v", o._statusCode, o.Payload)
}

func (o *PostIPIPGameIPOnGameRuleDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.PostIPIPGameIPOnGameRuleDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
