// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/ip/models"
)

// GetIPIPArpReader is a Reader for the GetIPIPArp structure.
type GetIPIPArpReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *GetIPIPArpReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewGetIPIPArpOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewGetIPIPArpDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewGetIPIPArpOK creates a GetIPIPArpOK with default headers values
func NewGetIPIPArpOK() *GetIPIPArpOK {
	return &GetIPIPArpOK{}
}

/*GetIPIPArpOK handles this case with default header values.

return value
*/
type GetIPIPArpOK struct {
	Payload []string
}

func (o *GetIPIPArpOK) Error() string {
	return fmt.Sprintf("[GET /ip/{ip}/arp][%d] getIpIpArpOK  %+v", 200, o.Payload)
}

func (o *GetIPIPArpOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	// response payload
	if err := consumer.Consume(response.Body(), &o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewGetIPIPArpDefault creates a GetIPIPArpDefault with default headers values
func NewGetIPIPArpDefault(code int) *GetIPIPArpDefault {
	return &GetIPIPArpDefault{
		_statusCode: code,
	}
}

/*GetIPIPArpDefault handles this case with default header values.

Unexpected error
*/
type GetIPIPArpDefault struct {
	_statusCode int

	Payload *models.GetIPIPArpDefaultBody
}

// Code gets the status code for the get IP IP arp default response
func (o *GetIPIPArpDefault) Code() int {
	return o._statusCode
}

func (o *GetIPIPArpDefault) Error() string {
	return fmt.Sprintf("[GET /ip/{ip}/arp][%d] GetIPIPArp default  %+v", o._statusCode, o.Payload)
}

func (o *GetIPIPArpDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.GetIPIPArpDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
