// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/ip/models"
)

// PostIPIPArpIPBlockedUnblockReader is a Reader for the PostIPIPArpIPBlockedUnblock structure.
type PostIPIPArpIPBlockedUnblockReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *PostIPIPArpIPBlockedUnblockReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewPostIPIPArpIPBlockedUnblockOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewPostIPIPArpIPBlockedUnblockDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewPostIPIPArpIPBlockedUnblockOK creates a PostIPIPArpIPBlockedUnblockOK with default headers values
func NewPostIPIPArpIPBlockedUnblockOK() *PostIPIPArpIPBlockedUnblockOK {
	return &PostIPIPArpIPBlockedUnblockOK{}
}

/*PostIPIPArpIPBlockedUnblockOK handles this case with default header values.

return 'void'
*/
type PostIPIPArpIPBlockedUnblockOK struct {
}

func (o *PostIPIPArpIPBlockedUnblockOK) Error() string {
	return fmt.Sprintf("[POST /ip/{ip}/arp/{ipBlocked}/unblock][%d] postIpIpArpIpBlockedUnblockOK ", 200)
}

func (o *PostIPIPArpIPBlockedUnblockOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	return nil
}

// NewPostIPIPArpIPBlockedUnblockDefault creates a PostIPIPArpIPBlockedUnblockDefault with default headers values
func NewPostIPIPArpIPBlockedUnblockDefault(code int) *PostIPIPArpIPBlockedUnblockDefault {
	return &PostIPIPArpIPBlockedUnblockDefault{
		_statusCode: code,
	}
}

/*PostIPIPArpIPBlockedUnblockDefault handles this case with default header values.

Unexpected error
*/
type PostIPIPArpIPBlockedUnblockDefault struct {
	_statusCode int

	Payload *models.PostIPIPArpIPBlockedUnblockDefaultBody
}

// Code gets the status code for the post IP IP arp IP blocked unblock default response
func (o *PostIPIPArpIPBlockedUnblockDefault) Code() int {
	return o._statusCode
}

func (o *PostIPIPArpIPBlockedUnblockDefault) Error() string {
	return fmt.Sprintf("[POST /ip/{ip}/arp/{ipBlocked}/unblock][%d] PostIPIPArpIPBlockedUnblock default  %+v", o._statusCode, o.Payload)
}

func (o *PostIPIPArpIPBlockedUnblockDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.PostIPIPArpIPBlockedUnblockDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
