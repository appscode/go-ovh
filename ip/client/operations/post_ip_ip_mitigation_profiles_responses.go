// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/ip/models"
)

// PostIPIPMitigationProfilesReader is a Reader for the PostIPIPMitigationProfiles structure.
type PostIPIPMitigationProfilesReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *PostIPIPMitigationProfilesReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewPostIPIPMitigationProfilesOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewPostIPIPMitigationProfilesDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewPostIPIPMitigationProfilesOK creates a PostIPIPMitigationProfilesOK with default headers values
func NewPostIPIPMitigationProfilesOK() *PostIPIPMitigationProfilesOK {
	return &PostIPIPMitigationProfilesOK{}
}

/*PostIPIPMitigationProfilesOK handles this case with default header values.

description of 'ip.MitigationProfile' response
*/
type PostIPIPMitigationProfilesOK struct {
	Payload *models.IPMitigationProfile
}

func (o *PostIPIPMitigationProfilesOK) Error() string {
	return fmt.Sprintf("[POST /ip/{ip}/mitigationProfiles][%d] postIpIpMitigationProfilesOK  %+v", 200, o.Payload)
}

func (o *PostIPIPMitigationProfilesOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.IPMitigationProfile)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewPostIPIPMitigationProfilesDefault creates a PostIPIPMitigationProfilesDefault with default headers values
func NewPostIPIPMitigationProfilesDefault(code int) *PostIPIPMitigationProfilesDefault {
	return &PostIPIPMitigationProfilesDefault{
		_statusCode: code,
	}
}

/*PostIPIPMitigationProfilesDefault handles this case with default header values.

Unexpected error
*/
type PostIPIPMitigationProfilesDefault struct {
	_statusCode int

	Payload *models.PostIPIPMitigationProfilesDefaultBody
}

// Code gets the status code for the post IP IP mitigation profiles default response
func (o *PostIPIPMitigationProfilesDefault) Code() int {
	return o._statusCode
}

func (o *PostIPIPMitigationProfilesDefault) Error() string {
	return fmt.Sprintf("[POST /ip/{ip}/mitigationProfiles][%d] PostIPIPMitigationProfiles default  %+v", o._statusCode, o.Payload)
}

func (o *PostIPIPMitigationProfilesDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.PostIPIPMitigationProfilesDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
