// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/ip/models"
)

// PostIPServiceServiceNameTerminateReader is a Reader for the PostIPServiceServiceNameTerminate structure.
type PostIPServiceServiceNameTerminateReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *PostIPServiceServiceNameTerminateReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewPostIPServiceServiceNameTerminateOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewPostIPServiceServiceNameTerminateDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewPostIPServiceServiceNameTerminateOK creates a PostIPServiceServiceNameTerminateOK with default headers values
func NewPostIPServiceServiceNameTerminateOK() *PostIPServiceServiceNameTerminateOK {
	return &PostIPServiceServiceNameTerminateOK{}
}

/*PostIPServiceServiceNameTerminateOK handles this case with default header values.

return value
*/
type PostIPServiceServiceNameTerminateOK struct {
	Payload string
}

func (o *PostIPServiceServiceNameTerminateOK) Error() string {
	return fmt.Sprintf("[POST /ip/service/{serviceName}/terminate][%d] postIpServiceServiceNameTerminateOK  %+v", 200, o.Payload)
}

func (o *PostIPServiceServiceNameTerminateOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	// response payload
	if err := consumer.Consume(response.Body(), &o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewPostIPServiceServiceNameTerminateDefault creates a PostIPServiceServiceNameTerminateDefault with default headers values
func NewPostIPServiceServiceNameTerminateDefault(code int) *PostIPServiceServiceNameTerminateDefault {
	return &PostIPServiceServiceNameTerminateDefault{
		_statusCode: code,
	}
}

/*PostIPServiceServiceNameTerminateDefault handles this case with default header values.

Unexpected error
*/
type PostIPServiceServiceNameTerminateDefault struct {
	_statusCode int

	Payload *models.PostIPServiceServiceNameTerminateDefaultBody
}

// Code gets the status code for the post IP service service name terminate default response
func (o *PostIPServiceServiceNameTerminateDefault) Code() int {
	return o._statusCode
}

func (o *PostIPServiceServiceNameTerminateDefault) Error() string {
	return fmt.Sprintf("[POST /ip/service/{serviceName}/terminate][%d] PostIPServiceServiceNameTerminate default  %+v", o._statusCode, o.Payload)
}

func (o *PostIPServiceServiceNameTerminateDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.PostIPServiceServiceNameTerminateDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
