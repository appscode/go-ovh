// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/ip/models"
)

// GetIPIPTaskReader is a Reader for the GetIPIPTask structure.
type GetIPIPTaskReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *GetIPIPTaskReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewGetIPIPTaskOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewGetIPIPTaskDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewGetIPIPTaskOK creates a GetIPIPTaskOK with default headers values
func NewGetIPIPTaskOK() *GetIPIPTaskOK {
	return &GetIPIPTaskOK{}
}

/*GetIPIPTaskOK handles this case with default header values.

return value
*/
type GetIPIPTaskOK struct {
	Payload []int64
}

func (o *GetIPIPTaskOK) Error() string {
	return fmt.Sprintf("[GET /ip/{ip}/task][%d] getIpIpTaskOK  %+v", 200, o.Payload)
}

func (o *GetIPIPTaskOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	// response payload
	if err := consumer.Consume(response.Body(), &o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewGetIPIPTaskDefault creates a GetIPIPTaskDefault with default headers values
func NewGetIPIPTaskDefault(code int) *GetIPIPTaskDefault {
	return &GetIPIPTaskDefault{
		_statusCode: code,
	}
}

/*GetIPIPTaskDefault handles this case with default header values.

Unexpected error
*/
type GetIPIPTaskDefault struct {
	_statusCode int

	Payload *models.GetIPIPTaskDefaultBody
}

// Code gets the status code for the get IP IP task default response
func (o *GetIPIPTaskDefault) Code() int {
	return o._statusCode
}

func (o *GetIPIPTaskDefault) Error() string {
	return fmt.Sprintf("[GET /ip/{ip}/task][%d] GetIPIPTask default  %+v", o._statusCode, o.Payload)
}

func (o *GetIPIPTaskDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.GetIPIPTaskDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
