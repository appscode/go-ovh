// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/ip/models"
)

// GetIPIPReverseIPReverseReader is a Reader for the GetIPIPReverseIPReverse structure.
type GetIPIPReverseIPReverseReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *GetIPIPReverseIPReverseReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewGetIPIPReverseIPReverseOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewGetIPIPReverseIPReverseDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewGetIPIPReverseIPReverseOK creates a GetIPIPReverseIPReverseOK with default headers values
func NewGetIPIPReverseIPReverseOK() *GetIPIPReverseIPReverseOK {
	return &GetIPIPReverseIPReverseOK{}
}

/*GetIPIPReverseIPReverseOK handles this case with default header values.

description of 'ip.ReverseIp' response
*/
type GetIPIPReverseIPReverseOK struct {
	Payload *models.IPReverseIP
}

func (o *GetIPIPReverseIPReverseOK) Error() string {
	return fmt.Sprintf("[GET /ip/{ip}/reverse/{ipReverse}][%d] getIpIpReverseIpReverseOK  %+v", 200, o.Payload)
}

func (o *GetIPIPReverseIPReverseOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.IPReverseIP)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewGetIPIPReverseIPReverseDefault creates a GetIPIPReverseIPReverseDefault with default headers values
func NewGetIPIPReverseIPReverseDefault(code int) *GetIPIPReverseIPReverseDefault {
	return &GetIPIPReverseIPReverseDefault{
		_statusCode: code,
	}
}

/*GetIPIPReverseIPReverseDefault handles this case with default header values.

Unexpected error
*/
type GetIPIPReverseIPReverseDefault struct {
	_statusCode int

	Payload *models.GetIPIPReverseIPReverseDefaultBody
}

// Code gets the status code for the get IP IP reverse IP reverse default response
func (o *GetIPIPReverseIPReverseDefault) Code() int {
	return o._statusCode
}

func (o *GetIPIPReverseIPReverseDefault) Error() string {
	return fmt.Sprintf("[GET /ip/{ip}/reverse/{ipReverse}][%d] GetIPIPReverseIPReverse default  %+v", o._statusCode, o.Payload)
}

func (o *GetIPIPReverseIPReverseDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.GetIPIPReverseIPReverseDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
