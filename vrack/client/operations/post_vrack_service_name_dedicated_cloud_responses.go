// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/vrack/models"
)

// PostVrackServiceNameDedicatedCloudReader is a Reader for the PostVrackServiceNameDedicatedCloud structure.
type PostVrackServiceNameDedicatedCloudReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *PostVrackServiceNameDedicatedCloudReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewPostVrackServiceNameDedicatedCloudOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewPostVrackServiceNameDedicatedCloudDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewPostVrackServiceNameDedicatedCloudOK creates a PostVrackServiceNameDedicatedCloudOK with default headers values
func NewPostVrackServiceNameDedicatedCloudOK() *PostVrackServiceNameDedicatedCloudOK {
	return &PostVrackServiceNameDedicatedCloudOK{}
}

/*PostVrackServiceNameDedicatedCloudOK handles this case with default header values.

description of 'vrack.Task' response
*/
type PostVrackServiceNameDedicatedCloudOK struct {
	Payload *models.VrackTask
}

func (o *PostVrackServiceNameDedicatedCloudOK) Error() string {
	return fmt.Sprintf("[POST /vrack/{serviceName}/dedicatedCloud][%d] postVrackServiceNameDedicatedCloudOK  %+v", 200, o.Payload)
}

func (o *PostVrackServiceNameDedicatedCloudOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.VrackTask)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewPostVrackServiceNameDedicatedCloudDefault creates a PostVrackServiceNameDedicatedCloudDefault with default headers values
func NewPostVrackServiceNameDedicatedCloudDefault(code int) *PostVrackServiceNameDedicatedCloudDefault {
	return &PostVrackServiceNameDedicatedCloudDefault{
		_statusCode: code,
	}
}

/*PostVrackServiceNameDedicatedCloudDefault handles this case with default header values.

Unexpected error
*/
type PostVrackServiceNameDedicatedCloudDefault struct {
	_statusCode int

	Payload *models.PostVrackServiceNameDedicatedCloudDefaultBody
}

// Code gets the status code for the post vrack service name dedicated cloud default response
func (o *PostVrackServiceNameDedicatedCloudDefault) Code() int {
	return o._statusCode
}

func (o *PostVrackServiceNameDedicatedCloudDefault) Error() string {
	return fmt.Sprintf("[POST /vrack/{serviceName}/dedicatedCloud][%d] PostVrackServiceNameDedicatedCloud default  %+v", o._statusCode, o.Payload)
}

func (o *PostVrackServiceNameDedicatedCloudDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.PostVrackServiceNameDedicatedCloudDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
