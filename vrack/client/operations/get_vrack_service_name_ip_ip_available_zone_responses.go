// Code generated by go-swagger; DO NOT EDIT.

// Copyright 2017 The go-ovh Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package operations

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-openapi/runtime"

	strfmt "github.com/go-openapi/strfmt"

	"github.com/appscode/go-ovh/vrack/models"
)

// GetVrackServiceNameIPIPAvailableZoneReader is a Reader for the GetVrackServiceNameIPIPAvailableZone structure.
type GetVrackServiceNameIPIPAvailableZoneReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *GetVrackServiceNameIPIPAvailableZoneReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewGetVrackServiceNameIPIPAvailableZoneOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		result := NewGetVrackServiceNameIPIPAvailableZoneDefault(response.Code())
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		if response.Code()/100 == 2 {
			return result, nil
		}
		return nil, result
	}
}

// NewGetVrackServiceNameIPIPAvailableZoneOK creates a GetVrackServiceNameIPIPAvailableZoneOK with default headers values
func NewGetVrackServiceNameIPIPAvailableZoneOK() *GetVrackServiceNameIPIPAvailableZoneOK {
	return &GetVrackServiceNameIPIPAvailableZoneOK{}
}

/*GetVrackServiceNameIPIPAvailableZoneOK handles this case with default header values.

return value
*/
type GetVrackServiceNameIPIPAvailableZoneOK struct {
	Payload []string
}

func (o *GetVrackServiceNameIPIPAvailableZoneOK) Error() string {
	return fmt.Sprintf("[GET /vrack/{serviceName}/ip/{ip}/availableZone][%d] getVrackServiceNameIpIpAvailableZoneOK  %+v", 200, o.Payload)
}

func (o *GetVrackServiceNameIPIPAvailableZoneOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	// response payload
	if err := consumer.Consume(response.Body(), &o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}

// NewGetVrackServiceNameIPIPAvailableZoneDefault creates a GetVrackServiceNameIPIPAvailableZoneDefault with default headers values
func NewGetVrackServiceNameIPIPAvailableZoneDefault(code int) *GetVrackServiceNameIPIPAvailableZoneDefault {
	return &GetVrackServiceNameIPIPAvailableZoneDefault{
		_statusCode: code,
	}
}

/*GetVrackServiceNameIPIPAvailableZoneDefault handles this case with default header values.

Unexpected error
*/
type GetVrackServiceNameIPIPAvailableZoneDefault struct {
	_statusCode int

	Payload *models.GetVrackServiceNameIPIPAvailableZoneDefaultBody
}

// Code gets the status code for the get vrack service name IP IP available zone default response
func (o *GetVrackServiceNameIPIPAvailableZoneDefault) Code() int {
	return o._statusCode
}

func (o *GetVrackServiceNameIPIPAvailableZoneDefault) Error() string {
	return fmt.Sprintf("[GET /vrack/{serviceName}/ip/{ip}/availableZone][%d] GetVrackServiceNameIPIPAvailableZone default  %+v", o._statusCode, o.Payload)
}

func (o *GetVrackServiceNameIPIPAvailableZoneDefault) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models.GetVrackServiceNameIPIPAvailableZoneDefaultBody)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
